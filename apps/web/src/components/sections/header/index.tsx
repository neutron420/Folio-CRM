...
export const Header = ({
  ...
  i18n = false,
  finalLinks,
}: Omit<HomeLayoutProps, 'nav'> & {
  nav?: (HomeLayoutProps['nav'] & { enableSearch?: boolean; [key: string]: any }) | Record<string, any>;
  finalLinks: LinkItemType[];
}) => {
  const { enableSearch = true, ...navProps } = nav;
...
const NavbarLinkItem = ({
  ...
  if (item.type === 'menu') {
    const children = item.items.map((child, j) => {
      ...
      const { banner, footer, ...rest } = (child.menu ?? {}) as any;
